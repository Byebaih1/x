...
return function(props)
	...
	AcrylicPaint.Frame = New("Frame", {
		...
		New("ImageLabel", {
			...
			BackgroundTransparency = 1,
			ImageColor3 = Color3.fromRGB(0, 10, 30),
			ImageTransparency = 0.6,
		}),

		New("UICorner", {
			CornerRadius = UDim.new(0, 12), -- ขอบโค้งมากขึ้น แตกต่างจาก Fluent เดิม (8)
		}),

		-- พื้นหลังหลัก (โทนน้ำเงิน)
		New("Frame", {
		...
		-- Gradient ฟ้า-น้ำเงินเข้ม
		New("Frame", {
			BackgroundColor3 = Color3.fromRGB(30, 145, 255),
		...
		-- Border เรืองแสงสีฟ้า (เด่นกว่า Fluent เดิมมาก)
		New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.fromScale(1, 1),
			ZIndex = 2,
...
return function(Config)
	...
	local LogoImage = New("ImageLabel", {
		Image = "rbxassetid://10618529241", -- Sleek Modern Shield/Logo
		Size = UDim2.fromOffset(20, 20),
		BackgroundTransparency = 1,
		Parent = TitleBar.Frame:FindFirstChild("Frame"), -- The first inner frame
		LayoutOrder = -1, -- Ensure it's at the front
	})

	if Config.Logo then
		if Config.Logo:find("http") then
			task.spawn(function()
				local ok, raw = pcall(game.HttpGet, game, Config.Logo, true)
				if ok then
					pcall(writefile, "raron_logo.png", raw)
					local fn = type(getcustomasset) == "function" and getcustomasset or type(getsynasset) == "function" and getsynasset
					if fn then
						local asset = fn("raron_logo.png")
						if asset then
		...
		Creator.AddThemeObject(LogoImage, {
			ImageColor3 = "Accent",
		})
	end
	...
	New("TextLabel", {
		...
		BackgroundTransparency = 1,
		Parent = TitleBar.Frame:FindFirstChild("Frame"),
		ThemeTag = {
			TextColor3 = "Text",
		},
	})
...
local ImageBanner = {}
ImageBanner.__index = ImageBanner
ImageBanner.__type = "ImageBanner"

--[[
    ใช้งาน:
    Tabs.Main:AddImageBanner({
        Url         = "https://i.imgur.com/XYZ.png",  -- URL รูปภาพ
        Height      = 120,    -- ความสูง (pixels)
        FileName    = "banner.png",  -- ชื่อไฟล์ที่บันทึก (optional)
        ScaleType   = "Crop", -- "Crop" / "Fit" / "Stretch"
        Transparency = 0,     -- 0 = ทึบ, 1 = โปร่งใส
    })
]]
...
function ImageBanner:New(Config)
    local success, err = pcall(function()
        assert(Config.Url, "ImageBanner - Missing Url")
        ...
        task.spawn(function()
            ...
            if not ok or not rawData or rawData == "" then
            ...
            if not writeOk then
                LoadingLabel.Text = "❌  Failed to save file"
                LoadingLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
                return
            end

            local getCustomAssetFunc = type(getcustomasset) == "function" and getcustomasset or type(getsynasset) == "function" and getsynasset or nil
            if not getCustomAssetFunc then
                LoadingLabel.Text = "⚠️  Executor unsupported"
                LoadingLabel.TextColor3 = Color3.fromRGB(255, 200, 50)
                return
            end

            local assetUrl = getCustomAssetFunc(fileName)
            if assetUrl then
        ...
        function BannerObj:SetUrl(newUrl, newFileName)
            newFileName = newFileName or fileName
            LoadingLabel.Visible = true
            LoadingLabel.Text = "⏳  Loading..."
            LoadingLabel.TextColor3 = Color3.fromRGB(140, 190, 255)

            task.spawn(function()
                local ok, rawData = pcall(function()
                    return game:HttpGet(newUrl, true)
                end)
                if not ok then return end
                pcall(function() writefile(newFileName, rawData) end)
                local fn = type(getcustomasset) == "function" and getcustomasset or type(getsynasset) == "function" and getsynasset or nil
                if fn then
                    local asset = fn(newFileName)
                    if asset then
    ...
    if not success then
        warn("ImageBanner Error: " .. tostring(err))
...
return Themes
	...
	AcrylicMain = Color3.fromRGB(15, 15, 18), -- Deeper dark, slightly cool
	AcrylicBorder = Color3.fromRGB(40, 40, 45),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(10, 10, 12), Color3.fromRGB(5, 5, 5)),
	AcrylicNoise = 0.98, -- More transparent noise

	TitleBarLine = Color3.fromRGB(45, 45, 50),
	Tab = Color3.fromRGB(120, 120, 130),

	Element = Color3.fromRGB(20, 20, 25),
	ElementBorder = Color3.fromRGB(10, 10, 12),
	InElementBorder = Color3.fromRGB(35, 35, 40),
	ElementTransparency = 0.65, -- More glassmorphism

	DropdownFrame = Color3.fromRGB(100, 100, 110),
	DropdownHolder = Color3.fromRGB(25, 25, 30),
	DropdownBorder = Color3.fromRGB(15, 15, 18),

	Dialog = Color3.fromRGB(25, 25, 30),
	DialogHolder = Color3.fromRGB(15, 15, 18),
	DialogHolderLine = Color3.fromRGB(10, 10, 12),
	DialogButton = Color3.fromRGB(20, 20, 25),
	DialogButtonBorder = Color3.fromRGB(35, 35, 40),
	DialogBorder = Color3.fromRGB(40, 40, 45),
	DialogInput = Color3.fromRGB(30, 30, 35),
	DialogInputLine = Color3.fromRGB(80, 80, 90),
}
 end,
	...
	-- ฟ้าเข้มสดใส เป็น Accent หลัก
	Accent = Color3.fromRGB(30, 145, 255),

	-- Acrylic: พื้นหลังขาวขุ่น Glassmorphism แบบ Light-Dark Fusion
	AcrylicMain = Color3.fromRGB(8, 14, 28),
	AcrylicBorder = Color3.fromRGB(30, 145, 255),
	AcrylicGradient = ColorSequence.new(Color3.fromRGB(10, 20, 45), Color3.fromRGB(4, 10, 22)),
	AcrylicNoise = 0.97,

	-- TitleBar
	TitleBarLine = Color3.fromRGB(30, 145, 255),
	Tab = Color3.fromRGB(180, 210, 255),

	-- Elements: สีขาว/ฟ้าใสสะอาด
	Element = Color3.fromRGB(20, 60, 120),
	ElementBorder = Color3.fromRGB(30, 145, 255),
	InElementBorder = Color3.fromRGB(50, 130, 220),
	ElementTransparency = 0.70,

	ToggleSlider = Color3.fromRGB(50, 130, 220),
	ToggleToggled = Color3.fromRGB(255, 255, 255),

	SliderRail = Color3.fromRGB(30, 80, 180),

	DropdownFrame = Color3.fromRGB(140, 190, 255),
	DropdownHolder = Color3.fromRGB(8, 22, 50),
	DropdownBorder = Color3.fromRGB(30, 145, 255),
	DropdownOption = Color3.fromRGB(20, 80, 180),

	Keybind = Color3.fromRGB(30, 145, 255),

	Input = Color3.fromRGB(15, 50, 110),
	InputFocused = Color3.fromRGB(5, 20, 55),
	InputIndicator = Color3.fromRGB(100, 180, 255),

	-- Dialog: โทนน้ำเงินเข้ม
	Dialog = Color3.fromRGB(10, 25, 55),
	DialogHolder = Color3.fromRGB(6, 16, 38),
	DialogHolderLine = Color3.fromRGB(30, 145, 255),
	DialogButton = Color3.fromRGB(15, 50, 110),
	DialogButtonBorder = Color3.fromRGB(30, 145, 255),
	DialogBorder = Color3.fromRGB(30, 145, 255),
	DialogInput = Color3.fromRGB(12, 35, 80),
	DialogInputLine = Color3.fromRGB(80, 160, 255),

	-- Text: ขาวสะอาด / ฟ้าอ่อน
	Text = Color3.fromRGB(240, 248, 255),
	SubText = Color3.fromRGB(140, 190, 255),
	Hover = Color3.fromRGB(30, 145, 255),
	HoverChange = 0.08,
}
 end,
